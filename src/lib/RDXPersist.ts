import { deleteKeypath, setKeypath } from '../utils';
...
export class RDXPersist<T> {
	...
	async persist(state: T) {
		...
		this.storage.setItem(
			...
			JSON.stringify(dataToBePersisted)
		);
	...
	private runMigrationsIfNeeded(state: Partial<T>) {
		...
		let migrationsRan = parseInt(
			this.storage.getItem(this.migrationsRanKey) || '-1'
		);

		// run migrations
		const migrationsThatNeedToBeRun = this.migrations.filter(
			(m) => m.number > migrationsRan
		);
